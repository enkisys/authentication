...
class sip2
{
	...
	function msgTLCLogin($sipLogin, $sipPassword, $sipLocation)
	{
		/* Login (93) - untested */
		$this->_newMessage('93');
		$this->_addFixedOption($this->UIDalgorithm, 1);
	...
	function getcrc($thecode)
	{
		...
		$len = strlen($thecode);
		for ($n = 0; $n < $len; $n++) {
			$sum = $sum + ord(substr($thecode, $n, 1));
		}
	...
	/*	logThis($message,'test','sip','enki_SIP_log');*/
		if ($this->debug) {
	...
	/*	logThis($test,'test','crc','enki_SIP_log');*/
		if (trim($test) == '') {
			return true;
		} else {
